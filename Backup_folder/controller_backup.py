...
import time
import enum # For creating enumerated constants (like a list of named options)
import math
import numpy as np

class Mode(enum.IntEnum): # Using them as IntEnum allows us to use them as integers directly
    STANDBY = 0xFF,
    EXTERNALPRINT = 0x06,
    TESTPATTERN = 0x01
...
class DLPC1438:
    ...
    def __init__(self, i2c_bus, PROJ_ON_PIN, HOST_IRQ_pin):
        ...
        self.PROJ_ON = PROJ_ON_PIN  # the PROJ_ON pin is used to turn on the DLPC1438
        self.HOST_IRQ = HOST_IRQ_pin  # the HOST_IRQ pin is used to signal that the DLPC1438 is ready for use

        # Configure the pins
        GPIO.setup(self.PROJ_ON, GPIO.OUT)  # set PROJ_ON pin as output
        GPIO.setup(self.HOST_IRQ, GPIO.IN)  # set HOST_IRQ pin as input

        self.i2c = i2c_bus  # the i2c bus to communicate with the DLPC1438
